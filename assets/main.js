...
database.on("child_added", function(trainshot){
    ...
    var trainName = $("<div>").attr("class", "col-2").text(tv.trainName);
    var destination = $("<div>").attr("class", "col-2").text(tv.destination);
    ...
    for (timeOfLastTrain = todayFirstTrainUnix; timeOfLastTrain < nowUnix; timeOfLastTrain += frequencySeconds);
    ...
    var frequency = $("<div>").attr("class", "col-2").text(tv.frequency);
    var nextArrival = $("<div>").attr("class", "col-2").text(nextArrivalReadable);
    var minutesAway = $("<div>").attr("class", "col-2").text(minutesAwayReadable);

    // Create extra column and append everthing to the train schedule
    var extraDivSecond = $("<div>").attr("class", "col-1");
    row.append(extraDivFirst, trainName, destination, frequency, nextArrival, minutesAway, extraDivSecond,);
